...
=clingon= supports named positional arguments, which allow you to
define named positions for free arguments and access them via
=CLINGON:GETOPT=, rather than manually indexing into
=CLINGON:COMMAND-ARGUMENTS=.

#+begin_src lisp
(defun copy/handler (cmd)
  "Handler for the `copy' command"
  (let ((source (clingon:getopt cmd :source))
        (dest (clingon:getopt cmd :dest))
        (force (clingon:getopt cmd :force)))
    (format t "Copying ~A -> ~A (force: ~A)~%" source dest force)))

(defun copy/command ()
  "A command which demonstrates named positional arguments"
  (clingon:make-command
   :name "copy"
...
When users invoke the command, positional arguments are assigned in
order:

#+begin_src shell
$ copy --force foo.txt bar.txt
Copying foo.txt -> bar.txt (force: T)
#+end_src

Positional arguments can be required or optional. Optional arguments
support default values via =:initial-value=. Any extra arguments
beyond the defined positionals are collected in
=CLINGON:COMMAND-ARGUMENTS= as before.

The generated help output will include an =ARGUMENTS= section:

#+begin_src text
USAGE:
  copy [options] <source> <dest> [arguments ...]

ARGUMENTS:
  source  source file to copy (required)
